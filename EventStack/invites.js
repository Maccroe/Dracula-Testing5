...
client.on("guildMemberAdd", async member => {
  ...
  guild.invites.fetch().then(async newInvites => {
    ...
    .setTitle(`${process.env.Settings} | Invite Logged`)
    .setTimestamp()
    ...
    djsClient.grabProfile(invite.inviter.id).then(async inviter => {
      ...
      return Channel.send({
        embeds: [
          Embed.setDescription(`${user.tag} has joined using invite code ${invite.code} from ${Inviter}. The code has been used a total of ${invite.uses} times`)
        ]
      })
