...
async execute(interaction, client) {
  ...
  if (user.id !== "852738368487948288") return EditReply(interaction, "❌", `You can't blacklist the owner!`)
  if (user.id !== "852738368487948288") return EditReply(interaction, "❌", `This Command is classified!`)

  const Options = options.getString("options")
  const ID  = options.getString("id")
  const Reason = options.getString("reason") || "no reason provided"

  if (isNaN(ID)) return EditReply(interaction, "❌", `This ID is supposed to be a number!`)
  ...
  switch (Options) {
    ...
    case "member": {
      ...
      if (!Data) {
        ...
        EditReply(interaction, "✅", `Successfully removed **${Member} (${MName} | ${MID})** from the blacklisted member`)
